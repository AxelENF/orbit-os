...
import { PublicationResults } from "@/components/content/publication-results";
import type { PublicationTarget } from "@/lib/content/repository";
import { approveContentTarget, getContentRecord, recordManualPublicationDelivery, recordPublicationResult, submitFinalCopy } from "@/lib/content/client";
import type { ContentRecord, FinalCopySubmission, ManualPublicationDeliveryInput, PublicationResult } from "@/lib/content/repository";
import { hasSupabaseBrowserConfig } from "@/lib/supabase/client";
import {
  approveDemoTarget,
  recordDemoManualPublicationDelivery,
  recordDemoPublicationResult,
  readDemoDraft,
...
function DemoDraftEditor({ draftId }: DraftEditorProps) {
  ...
  function handlePublicationResult(
    ...
    if (!record) return Promise.reject(new Error("DEMO_RECORD_NOT_FOUND"));
    const next = recordDemoPublicationResult(record.content.id, targetId, input);
    const created = next?.publicationResults.at(0);
    if (!next || !created) return Promise.reject(new Error("DEMO_RESULT_NOT_RECORDED"));
    setRecord(next);
    setFeedback("Resultado observado guardado localmente; no se calculó atribución automática.");
        ...
        <div className="space-y-6">
          ...
          {targetsAreReviewable ? (
            <>
              ...
              <PublicationResults targets={record.targets} results={record.publicationResults ?? []} onRecord={handlePublicationResult} />
            </>
          ) : (
...
function ProductionDraftEditor({ draftId }: DraftEditorProps) {
  ...
  async function requestCopy() {
    if (!record) return;
    const idempotencyKey = copyRetryKey ?? crypto.randomUUID();
    setIsRequestingCopy(true);
    setFeedback(null);
    try {
      const response = await fetch(`/api/content/${record.content.id}/copy`, {
        method: "POST",
      ...
      setFeedback("Generación encolada en el motor interno.");
      await refresh();
    } catch {
      setCopyRetryKey(idempotencyKey);
      setFeedback("No se pudo confirmar la generación. Reintenta sin duplicar el trabajo.");
    } finally {
  ...
  async function recordResult(
    ...
    if (!record) throw new Error("CONTENT_RECORD_NOT_LOADED");
    const result = await recordPublicationResult({
      contentItemId: record.content.id,
      publicationTargetId: targetId,
      ...input,
    ...
    await refresh();
    setFeedback("Resultado observado guardado. La atribución comercial sigue siendo una decisión humana.");
    return result;
  }
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="grid gap-6 xl:grid-cols-[0.86fr_1.14fr]">
        ...
        <div className="space-y-6">
          ...
          <PublicationResults targets={record.targets} results={record.publicationResults} onRecord={recordResult} />
