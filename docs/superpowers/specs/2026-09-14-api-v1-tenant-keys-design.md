...
**Fecha:** 2026-09-14 (revisado 2026-09-15 tras 3 rondas de revisión de Codex CLI — política de esta sesión, máximo 3 rondas automatizadas)
...
declare
  ...
  v_id uuid;
begin
  -- Corrección ronda 3 (bug real de seguridad): has_organization_role
  -- devuelve NULL, no false, cuando el caller no tiene ninguna fila de
  -- membresía — y `if not null` en plpgsql es `if null`, que NO ejecuta el
  -- bloque. La versión anterior dejaba pasar a cualquier no-miembro sin
  -- levantar la excepción, permitiendo crear claves para una organización
  -- ajena. `coalesce(..., false)` fuerza NULL a "no autorizado".
...
- `POST /api/organizations/:id/api-keys` — body `{ label: string }`, llama a `create_organization_api_key`, responde `{ id, keyPrefix, secret }` (una sola vez). **Corrección ronda 3 (hallazgo real):** la RPC usa `returns table (...)` — PostgREST/el cliente de Supabase la expone como un arreglo de filas, no un objeto único, aunque solo haya una fila. La ruta debe desenvolver explícitamente el único elemento (`data[0]`) antes de dar forma a la respuesta HTTP, y tratar un resultado vacío o con más de una fila como `500` (no debería poder pasar, pero no se asume silenciosamente).
...
- **`POST /api/v1/campaigns/:id/publish`** — publica un target específico.
  ...
  **Corrección ronda 2 (clasificación de errores todavía incompleta en la ronda 1):** la ronda 1 solo distinguía 404 (no existe/no es de esta organización) de 409 (existe pero no está `APPROVED`), pero `preparePublishRequest` (`lib/supabase/repository.ts:1003`) colapsa un **error real de consulta** en el mismo `PublishTargetConflictError` que "no encontrado" — un fallo transitorio de base de datos no debe convertirse en un falso `409 TARGET_NOT_APPROVED`. La ruta distingue tres casos explícitamente con una consulta propia (`.maybeSingle()`, que sí distingue `{data: null, error: null}` de un `error` real — verificado contra el cliente instalado): (1) `error` presente → `503 TARGET_LOOKUP_FAILED`, comprobado **antes** que `!data`; (2) sin `error` pero `!data` → `404 TARGET_NOT_FOUND`; (3) fila encontrada pero `status !== "APPROVED"` → `409 TARGET_NOT_APPROVED`.

  **Corrección ronda 3 (hallazgo real — la ronda 2 no cerraba el problema del todo):** tras pasar limpio por (1)-(3), el código sigue llamando a `requestN8nPublish`, que internamente vuelve a invocar `preparePublishRequest` (`lib/integrations/n8n-client.ts:197-201`) — una SEGUNDA consulta, redundante, que conserva el mismo defecto de conflación (`lib/supabase/repository.ts:995-1007`). Sin tocar esa función compartida (la usa también la ruta interna de n8n), la ruta de esta API captura específicamente `PublishTargetConflictError` proveniente de esa segunda llamada y, dado que el pre-chequeo propio ya confirmó segundos antes que el target existe, pertenece a esta organización y está `APPROVED`, lo reclasifica como `503 TARGET_LOOKUP_FAILED` (una condición de carrera o fallo transitorio en la ventana entre ambas consultas es la explicación más honesta en ese punto — no un `409` genuino, que ya habría aparecido en el paso (3) propio).
...
- **Corrección ronda 2 (inconsistencia real, no resuelta en la ronda 1):** la ronda 1 decía "el logo baja a 2MB" pero nunca comprometía el cambio concreto — `MAX_LOGO_BYTES` en `app/api/organizations/[id]/logo/route.ts:11` seguía en 5MB. Ahora es explícito: **esa constante cambia a `2 * 1024 * 1024`**, aplicada tanto al `POST` de subida como a la validación del objeto descargado para composición (el `GET`/download hoy no revalida tamaño ni decodificación en absoluto, `:88-100,214-217` — el flujo nuevo sí lo hace). Un logo ya subido por encima de 2MB antes de este cambio (borde teórico — no hay ningún logo real subido todavía en ningún entorno) se trata en la composición como fallo controlado (`503 LOGO_TOO_LARGE`), no como crash.
- **Corrección ronda 3 (hallazgo real):** `MAX_LOGO_BYTES` hoy es una constante privada, no exportada, del archivo de la ruta manual — y la composición server-side vive en un módulo/ruta distinto. "Un solo límite, un solo lugar de verdad" no se logra solo bajando el número; la constante se **exporta** desde `app/api/organizations/[id]/logo/route.ts` (o se mueve a `lib/logo-studio/compose.ts`, junto a `computeLogoPlacement`, si por convención de este proyecto las constantes compartidas viven con la lógica de dominio en vez de en una ruta) para que ambos puntos de uso importen el mismo símbolo — decisión exacta de ubicación para el plan, no para el spec.
...
- **Corrección ronda 2 (hallazgo real — el mecanismo de la ronda 1 no existía) + corrección ronda 3 (la ronda 2 seguía sin cerrar el circuito completo):** la ronda 1 proponía que `actor_id` fuera el `uuid` del owner y `metadata` cargara `{"via": "api_key", ...}`, y afirmaba que ninguna RPC necesitaba cambiar. Falso: las RPCs actuales insertan `metadata` con `jsonb_build_object(...)` de campos fijos, sin ningún parámetro para adjuntar contexto extra. La ronda 2 propuso agregar un parámetro opcional `p_actor_metadata jsonb default '{}'::jsonb`, pero la ronda 3 encontró dos problemas reales adicionales, ahora resueltos aquí:

  1. **Compatibilidad hacia atrás real:** en Postgres, `create or replace function` con un parámetro nuevo NO reemplaza la firma anterior — crea un overload, y la firma vieja queda viva junto a la nueva, con riesgo de ambigüedad para los callers existentes que no pasan el parámetro nuevo (`lib/supabase/repository.ts:588-600,796-806,970-975` invocan estas RPCs sin él hoy). Este mismo proyecto ya resolvió exactamente este problema antes: `supabase/migrations/0014_copy_hashtags_and_ai_usage.sql:348-350` hace `drop function if exists public.submit_final_copy_for_review(<firma vieja exacta>);` antes de crear la función con la firma nueva. Cada RPC tocada aquí sigue el mismo patrón: `drop function if exists` con la firma exacta actual, `create function` con la firma nueva (incluyendo `p_actor_metadata`), y se repiten los `revoke`/`grant execute` que ya tenía. El repositorio TypeScript se actualiza en el mismo commit para pasar el parámetro nuevo en sus llamadas existentes (con `{}` cuando la acción no viene de una API key), así no queda ningún caller apuntando a la firma vieja.
  2. **Qué RPCs tocar, con precisión (la ronda 2 decía "las relevantes" sin enumerarlas, y se equivocaba en una):** las que efectivamente insertan en `audit_events` como parte de una acción que la API v1 puede disparar: la RPC de creación de contenido (`supabase/migrations/0009_tenantize_content_and_jobs.sql:207-211`) y `enqueue_copy_automation_job` (`supabase/migrations/0010_automation_jobs.sql:108-112`, encontrada en la ronda 3 — la ronda 2 no la había listado). **`preparePublishRequest` (`lib/supabase/repository.ts:992-1017`) NO es una RPC de Postgres — es una consulta directa desde TypeScript, y no inserta ningún `audit_events` por sí misma** (corrección de una imprecisión de la ronda 2, que la daba por incluida implícitamente). Atribuir la acción de publicar a una API key específica, si se quiere, es un mecanismo aparte — no se resuelve en este spec ni bloquea el resto del diseño; queda anotado como extensión futura, igual que el diagnóstico de ADR-008.

  Para que `apiKeyId`/`apiKeyLabel` lleguen desde la verificación de la clave (sección de autenticación) hasta estas dos RPCs, `OrganizationContext` (`lib/organizations/context.ts:7-13`) gana un campo opcional `apiKey?: { id: string; label: string }`, poblado solo cuando el contexto se construyó desde una clave de API (nunca desde una sesión de navegador); los métodos del repositorio que llaman a estas dos RPCs leen ese campo y lo pasan como `p_actor_metadata` cuando está presente, `{}` si no. Cambio de tipo pequeño y aditivo — no rompe ningún caller de sesión existente, que simplemente nunca puebla ese campo.
...
- RPCs de creación de contenido y de encolado de job (`enqueue_copy_automation_job`) recreadas con `drop function if exists` + `create function` (firma nueva con `p_actor_metadata`, siguiendo el patrón ya usado en `0014_copy_hashtags_and_ai_usage.sql`) — un test confirma que no queda ninguna firma vieja resoluble por PostgREST, y que los callers de sesión existentes (sin ese parámetro) siguen funcionando igual que antes.
- `create_organization_api_key`/`revoke_organization_api_key` rechazan explícitamente a cualquier caller sin membresía en absoluto (no solo a un miembro con rol distinto de owner) — cubre el caso `has_organization_role` devolviendo `NULL`, no solo `false`.
