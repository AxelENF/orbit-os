  ...
  z.object({
    ...
    job: z.object({
      ...
      meta: z.object({
        ...
        instagramBusinessAccountId: z.string().min(1).nullable(),
      }),
...
export type SupabasePublishJobPayload = {
...
export type SupabasePublishWorkerStoreOptions = {
  signedUrlSeconds?: number;
  provider?: string;
};

type PublishDurableJob = DurableJob<SupabasePublishJobPayload, Record<string, unknown>>;
...
/**
 * Supabase adapter for the worker-only side of publish jobs. The Next route
 * uses a server-guarded re-export; this core module is intentionally
 * importable by the standalone Node worker.
 *
 * Enqueue remains portal-owned by the publication workflow; this adapter
 ...
 */
export class SupabasePublishWorkerStore
  implements DurableJobWorkerStore<SupabasePublishJobPayload, Record<string, unknown>>
{
  private readonly claimed = new Map<string, PublishDurableJob>();
  private readonly signedUrlSeconds: number;
  private readonly provider: string;

  constructor(
    private readonly client: SupabaseClient,
    options: SupabasePublishWorkerStoreOptions = {},
  ) {
  ...
  async claimNext(input: { organizationId?: string } = {}): Promise<DurableJobMutationResult<SupabasePublishJobPayload, Record<string, unknown>>> {
    if (input.organizationId !== undefined) requireUuid(input.organizationId, "organizationId");
    const { data, error } = await this.client.rpc("claim_next_publish_automation_job", {
      p_provider: this.provider,
      p_organization_id: input.organizationId ?? null,
    });
    if (error) throw new Error("Unable to claim a durable publish job.");
    const parsed = claimResponseSchema.safeParse(data);
    if (!parsed.success) throw new Error("Supabase returned an invalid durable publish job claim.");
    if (parsed.data.state !== "CLAIMED") return null;

    const signedAssets: SupabasePublishJobPayload["assets"] = [];
    for (const asset of parsed.data.job.assets) {
      const signed = await this.client.storage
        .from("content-assets")
        .createSignedUrl(asset.storagePath, this.signedUrlSeconds);
      if (signed.error || !signed.data?.signedUrl) {
        await this.failUncachedClaim(parsed.data.job).catch(() => undefined);
        throw new Error("Unable to create a signed publish-job asset URL.");
      }
      signedAssets.push({ ...asset, assetUrl: signed.data.signedUrl });
    }

    const now = new Date().toISOString();
    const job: PublishDurableJob = {
      ...
      payload: {
        ...
        publicationTargetId: parsed.data.job.publicationTargetId,
        platform: parsed.data.job.platform,
        assets: signedAssets,
        copy: clone(parsed.data.job.copy),
        meta: clone(parsed.data.job.meta),
      },
  ...
  async renewLease(input: RenewDurableJobLeaseInput): Promise<DurableJobMutationResult<SupabasePublishJobPayload, Record<string, unknown>>> {
    ...
    const { data, error } = await this.client.rpc("renew_publish_automation_job", {
      p_job_id: input.jobId,
      p_idempotency_key: claimedJob.idempotencyKey,
      p_lease_token: input.leaseToken,
      p_lease_seconds: leaseSeconds(input.leaseDurationMs),
    });
    if (error) throw new Error("Unable to renew the durable publish job lease.");
  ...
  async complete(input: CompleteDurableJobInput<Record<string, unknown>>): Promise<DurableJobMutationResult<SupabasePublishJobPayload, Record<string, unknown>>> {
    const job = this.requireClaim(input.jobId, input.organizationId, input.leaseToken);
    const { data, error } = await this.client.rpc("complete_publish_automation_job", {
      p_job_id: input.jobId,
    ...
    if (error) {
      if (errorMessage(error) === "PUBLISH_JOB_NOT_FOUND" || errorMessage(error) === "PUBLISH_JOB_LEASE_INVALID") {
        this.claimed.delete(input.jobId);
        return null;
      }
      throw new Error("Unable to complete the durable publish job.");
    }
    if (!z.object({ created: z.boolean() }).safeParse(data).success) {
      throw new Error("Supabase returned an invalid publish job completion.");
    }
  ...
  async fail(input: FailDurableJobInput): Promise<DurableJobMutationResult<SupabasePublishJobPayload, Record<string, unknown>>> {
    const job = this.requireClaim(input.jobId, input.organizationId, input.leaseToken);
    const { data, error } = await this.client.rpc("fail_publish_automation_job", {
      p_job_id: input.jobId,
      p_idempotency_key: job.idempotencyKey,
      p_lease_token: input.leaseToken,
    ...
    if (error) {
      ...
      throw new Error("Unable to fail the durable publish job.");
    }
    const parsed = failureResponseSchema.safeParse(data);
    if (!parsed.success) throw new Error("Supabase returned an invalid publish job failure.");
  ...
  async cancel(input: CancelDurableJobInput): Promise<DurableJobMutationResult<SupabasePublishJobPayload, Record<string, unknown>>> {
    requireUuid(input.jobId, "jobId");
    requireUuid(input.organizationId, "organizationId");
    ...
    const { data, error } = await this.client.rpc("cancel_publish_automation_job", {
      p_job_id: input.jobId,
    ...
    if (error) {
      if (errorMessage(error) === "PUBLISH_JOB_LEASE_INVALID") return null;
      throw new Error("Unable to cancel the durable publish job.");
    }
    const parsed = cancelResponseSchema.safeParse(data);
    if (!parsed.success) throw new Error("Supabase returned an invalid publish job cancellation.");
  ...
  async recoverExpired(limit = 100): Promise<number> {
    ...
    const { data, error } = await this.client.rpc("recover_expired_publish_automation_jobs", { p_limit: limit });
    if (error) throw new Error("Unable to recover expired durable publish jobs.");
  ...
  async health(): Promise<DurableJobHealth> {
    const { data, error } = await this.client.rpc("summarize_publish_automation_jobs");
    if (error) throw new Error("Unable to read durable publish worker health.");
  ...
  private requireClaim(jobId: string, organizationId: string, leaseToken: string): PublishDurableJob {
    ...
    if (!job || job.organizationId !== organizationId || job.leaseToken !== leaseToken) {
      throw new Error("The durable publish job lease is not owned by this worker.");
    }
    return job;
  }
  ...
  private async failUncachedClaim(job: {
    ...
    await this.client.rpc("fail_publish_automation_job", {
      p_job_id: job.id,
      p_idempotency_key: job.idempotencyKey,
...
export function createSupabasePublishWorkerStore(
  client: SupabaseClient,
  options?: SupabasePublishWorkerStoreOptions,
): SupabasePublishWorkerStore {
  return new SupabasePublishWorkerStore(client, options);
}
