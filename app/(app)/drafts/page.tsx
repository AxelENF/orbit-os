  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="flex flex-col gap-6 border-b border-white/[0.08] pb-8 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="font-mono text-[0.68rem] uppercase tracking-[0.24em] text-[#A8C7FF]/80">Borradores de copy</p>
          <h1 className="mt-3 max-w-2xl text-3xl font-semibold tracking-[-0.04em] text-white sm:text-4xl">Elige el mensaje antes de enviarlo a revisión.</h1>
        ...
        <Link href="/library/new" className="inline-flex min-h-12 items-center justify-center rounded-xl bg-[#FF4D00] px-5 text-sm font-bold text-white shadow-lg shadow-[#FF4D00]/15 transition hover:bg-[#ff6a2f]">+ Nueva campaña</Link>
      </div>

      {isLoading ? <p className="mt-8 text-sm text-slate-500">Cargando borradores locales…</p> : null}

      {!isLoading && isProductionMode && productionRecords.length > 0 ? (
        <section className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {productionRecords.map((record) => (
            <article key={record.content.id} className="overflow-hidden rounded-3xl border border-white/[0.08] bg-[#0b1429]">
              {record.asset?.signedUrl ? <img src={record.asset.signedUrl} alt={`Preview de ${record.asset.filename}`} className="aspect-[4/5] w-full object-cover" /> : <div className="flex aspect-[4/5] items-center justify-center bg-[#081127] text-xs text-slate-600">Asset sin URL temporal</div>}
              <div className="space-y-4 p-5">
                <div className="flex items-center justify-between gap-3">
                  <span className="rounded-full border border-cyan-200/20 bg-cyan-200/[0.05] px-2 py-1 font-mono text-[0.6rem] uppercase tracking-[0.12em] text-cyan-100/80">{statusLabels[record.content.state]}</span>
                  <span className="text-xs text-slate-500">Supabase</span>
                </div>
                <div>
                  <h2 className="truncate text-sm font-semibold text-white">{record.asset?.filename ?? "Creativo sin asset"}</h2>
                  <p className="mt-2 text-xs leading-5 text-slate-500">{record.content.service.replaceAll("_", " ")} · {record.content.niche.replaceAll("_", " ")} · {record.content.contentType.replaceAll("_", " ")}</p>
                </div>
                <div className="flex items-center justify-between border-t border-white/[0.08] pt-4 text-xs text-slate-500">
                  <span>{record.drafts.length} alternativas</span>
                  <span>{record.targets.filter((target) => target.status === "APPROVED").length}/2 destinos aprobados</span>
                </div>
                <Link href={`/drafts/${record.content.id}`} className="inline-flex min-h-10 w-full items-center justify-center rounded-lg border border-cyan-200/25 px-3 py-2 text-sm font-semibold text-cyan-100 transition hover:bg-cyan-200/10">Abrir borrador →</Link>
              </div>
            </article>
          ))}
        </section>
      ...
      {!isLoading && !isProductionMode && drafts.length > 0 ? (
        <section className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {drafts.map((draft) => (
      ...
      {!isLoading && ((isProductionMode && productionRecords.length === 0) || (!isProductionMode && drafts.length === 0)) ? (
        <section className="mt-8 rounded-3xl border border-dashed border-cyan-200/20 bg-cyan-200/[0.025] px-6 py-14 text-center sm:px-10">
          ...
          <p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-slate-400">{isProductionMode ? "Sube un creativo desde Canva y solicita el análisis a n8n cuando el asset esté listo." : "Sube un creativo desde Canva y completa el brief. En demo local se crearán dos opciones sin llamar a IA ni a servicios externos."}</p>
